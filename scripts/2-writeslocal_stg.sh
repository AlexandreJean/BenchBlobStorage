...
ssh $SSH_ARGS -i ./hpcadmin_id_rsa $admin_user@$headnode_fqdn "pdsh -f $numIONodes -w ^azhpc_install_config.vmsscluster/hostlists/compute ifconfig eth0" | awk '{if ($0 ~ /inet /) {print $2}}' | cut -d "." -f 3,4 | sed 's/\.//' | sort -n > nodelist.txt
